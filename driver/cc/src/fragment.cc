// fragment.cc
// UDP packet fragmentation and reassembly implementation

#include "fragment.h"

#include <algorithm>
#include <cstring>

namespace imujoco::driver {

// CRC-16-CCITT lookup table (polynomial 0x1021)
static const uint16_t kCRC16Table[256] = {
    0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50A5, 0x60C6, 0x70E7, 0x8108,
    0x9129, 0xA14A, 0xB16B, 0xC18C, 0xD1AD, 0xE1CE, 0xF1EF, 0x1231, 0x0210,
    0x3273, 0x2252, 0x52B5, 0x4294, 0x72F7, 0x62D6, 0x9339, 0x8318, 0xB37B,
    0xA35A, 0xD3BD, 0xC39C, 0xF3FF, 0xE3DE, 0x2462, 0x3443, 0x0420, 0x1401,
    0x64E6, 0x74C7, 0x44A4, 0x5485, 0xA56A, 0xB54B, 0x8528, 0x9509, 0xE5EE,
    0xF5CF, 0xC5AC, 0xD58D, 0x3653, 0x2672, 0x1611, 0x0630, 0x76D7, 0x66F6,
    0x5695, 0x46B4, 0xB75B, 0xA77A, 0x9719, 0x8738, 0xF7DF, 0xE7FE, 0xD79D,
    0xC7BC, 0x48C4, 0x58E5, 0x6886, 0x78A7, 0x0840, 0x1861, 0x2802, 0x3823,
    0xC9CC, 0xD9ED, 0xE98E, 0xF9AF, 0x8948, 0x9969, 0xA90A, 0xB92B, 0x5AF5,
    0x4AD4, 0x7AB7, 0x6A96, 0x1A71, 0x0A50, 0x3A33, 0x2A12, 0xDBFD, 0xCBDC,
    0xFBBF, 0xEB9E, 0x9B79, 0x8B58, 0xBB3B, 0xAB1A, 0x6CA6, 0x7C87, 0x4CE4,
    0x5CC5, 0x2C22, 0x3C03, 0x0C60, 0x1C41, 0xEDAE, 0xFD8F, 0xCDEC, 0xDDCD,
    0xAD2A, 0xBD0B, 0x8D68, 0x9D49, 0x7E97, 0x6EB6, 0x5ED5, 0x4EF4, 0x3E13,
    0x2E32, 0x1E51, 0x0E70, 0xFF9F, 0xEFBE, 0xDFDD, 0xCFFC, 0xBF1B, 0xAF3A,
    0x9F59, 0x8F78, 0x9188, 0x81A9, 0xB1CA, 0xA1EB, 0xD10C, 0xC12D, 0xF14E,
    0xE16F, 0x1080, 0x00A1, 0x30C2, 0x20E3, 0x5004, 0x4025, 0x7046, 0x6067,
    0x83B9, 0x9398, 0xA3FB, 0xB3DA, 0xC33D, 0xD31C, 0xE37F, 0xF35E, 0x02B1,
    0x1290, 0x22F3, 0x32D2, 0x4235, 0x5214, 0x6277, 0x7256, 0xB5EA, 0xA5CB,
    0x95A8, 0x8589, 0xF56E, 0xE54F, 0xD52C, 0xC50D, 0x34E2, 0x24C3, 0x14A0,
    0x0481, 0x7466, 0x6447, 0x5424, 0x4405, 0xA7DB, 0xB7FA, 0x8799, 0x97B8,
    0xE75F, 0xF77E, 0xC71D, 0xD73C, 0x26D3, 0x36F2, 0x0691, 0x16B0, 0x6657,
    0x7676, 0x4615, 0x5634, 0xD94C, 0xC96D, 0xF90E, 0xE92F, 0x99C8, 0x89E9,
    0xB98A, 0xA9AB, 0x5844, 0x4865, 0x7806, 0x6827, 0x18C0, 0x08E1, 0x3882,
    0x28A3, 0xCB7D, 0xDB5C, 0xEB3F, 0xFB1E, 0x8BF9, 0x9BD8, 0xABBB, 0xBB9A,
    0x4A75, 0x5A54, 0x6A37, 0x7A16, 0x0AF1, 0x1AD0, 0x2AB3, 0x3A92, 0xFD2E,
    0xED0F, 0xDD6C, 0xCD4D, 0xBDAA, 0xAD8B, 0x9DE8, 0x8DC9, 0x7C26, 0x6C07,
    0x5C64, 0x4C45, 0x3CA2, 0x2C83, 0x1CE0, 0x0CC1, 0xEF1F, 0xFF3E, 0xCF5D,
    0xDF7C, 0xAF9B, 0xBFBA, 0x8FD9, 0x9FF8, 0x6E17, 0x7E36, 0x4E55, 0x5E74,
    0x2E93, 0x3EB2, 0x0ED1, 0x1EF0};

uint16_t ComputeCRC16(const uint8_t* data, size_t length) {
    uint16_t crc = 0xFFFF;
    for (size_t i = 0; i < length; i++) {
        crc = (crc << 8) ^ kCRC16Table[((crc >> 8) ^ data[i]) & 0xFF];
    }
    return crc;
}

uint16_t ComputeHeaderChecksum(const FragmentHeader* header) {
    // Compute CRC over header bytes excluding the checksum field (last 2 bytes)
    return ComputeCRC16(reinterpret_cast<const uint8_t*>(header),
                        kFragmentHeaderSize - sizeof(uint16_t));
}

bool ValidateHeaderChecksum(const FragmentHeader* header) {
    if (!header) return false;
    if (header->magic != kFragmentMagic) return false;
    return header->checksum == ComputeHeaderChecksum(header);
}

// FragmentedSender implementation

std::vector<std::vector<uint8_t>> FragmentedSender::FragmentMessage(
    const uint8_t* data, size_t size) {
    std::vector<std::vector<uint8_t>> fragments;

    if (!data || size == 0) {
        return fragments;
    }

    if (size > kMaxMessageSize) {
        return fragments;  // Message too large
    }

    // Calculate number of fragments needed (safe division to avoid overflow)
    // size is already bounded by kMaxMessageSize check above
    uint8_t fragment_count = static_cast<uint8_t>(
        (size / kMaxFragmentPayload) + (size % kMaxFragmentPayload != 0 ? 1 : 0));

    // Get unique message ID
    uint16_t msg_id = message_id_++;

    fragments.reserve(fragment_count);

    size_t offset = 0;
    for (uint8_t i = 0; i < fragment_count; i++) {
        size_t payload_size =
            std::min(kMaxFragmentPayload, size - offset);

        // Create fragment buffer
        std::vector<uint8_t> fragment(kFragmentHeaderSize + payload_size);

        // Build fragment header
        FragmentHeader* header =
            reinterpret_cast<FragmentHeader*>(fragment.data());
        header->magic = kFragmentMagic;
        header->message_id = msg_id;
        header->fragment_index = i;
        header->fragment_count = fragment_count;
        header->total_size = static_cast<uint32_t>(size);
        header->payload_size = static_cast<uint16_t>(payload_size);
        header->checksum = 0;
        header->checksum = ComputeHeaderChecksum(header);

        // Copy payload
        std::memcpy(fragment.data() + kFragmentHeaderSize, data + offset,
                    payload_size);

        fragments.push_back(std::move(fragment));
        offset += payload_size;
    }

    return fragments;
}

// ReassemblyManager implementation

ReassemblyResult ReassemblyManager::ProcessFragment(const uint8_t* data,
                                                     size_t size) {
    ReassemblyResult result;

    if (!data) {
        return result;
    }

    // Validate minimum size
    if (size < kFragmentHeaderSize) {
        return result;
    }

    const FragmentHeader* header =
        reinterpret_cast<const FragmentHeader*>(data);

    // Validate magic
    if (header->magic != kFragmentMagic) {
        return result;
    }

    // Validate checksum
    if (!ValidateHeaderChecksum(header)) {
        return result;
    }

    // Validate header fields
    if (header->fragment_index >= header->fragment_count ||
        header->fragment_count == 0 ||
        header->total_size == 0 ||
        header->payload_size > kMaxFragmentPayload ||
        header->payload_size + kFragmentHeaderSize != size ||
        header->total_size > kMaxMessageSize) {
        return result;
    }

    // Validate fragment_count is consistent with total_size
    uint8_t expected_count = static_cast<uint8_t>(
        (header->total_size + kMaxFragmentPayload - 1) / kMaxFragmentPayload);
    if (header->fragment_count != expected_count) {
        return result;
    }

    // Find or create reassembly slot
    ReassemblySlot* slot = find_or_create_slot(header->message_id);
    if (!slot) {
        return result;
    }

    // Initialize slot if new
    if (!slot->active) {
        slot->message_id = header->message_id;
        slot->fragment_count = header->fragment_count;
        slot->total_size = header->total_size;
        slot->received_count = 0;
        slot->received_mask = 0;
        std::memset(slot->extended_mask, 0, sizeof(slot->extended_mask));
        slot->buffer.resize(header->total_size);
        slot->active = true;
    }

    // Validate consistency with existing slot
    if (slot->fragment_count != header->fragment_count ||
        slot->total_size != header->total_size) {
        return result;
    }

    // Check for duplicate fragment
    if (slot->IsFragmentReceived(header->fragment_index)) {
        // Duplicate - update activity time but don't process again
        slot->last_activity = Clock::now();
        return result;
    }

    // Mark fragment as received
    slot->MarkFragmentReceived(header->fragment_index);
    slot->received_count++;
    slot->last_activity = Clock::now();

    // Copy payload to correct position in buffer
    size_t offset =
        static_cast<size_t>(header->fragment_index) * kMaxFragmentPayload;
    const uint8_t* payload = data + kFragmentHeaderSize;

    // Validate that payload fits within total message buffer
    if (offset + header->payload_size > slot->total_size) {
        return result;
    }

    std::memcpy(slot->buffer.data() + offset, payload, header->payload_size);

    // Check if complete
    if (slot->received_count == slot->fragment_count) {
        result.complete = true;
        result.data = slot->buffer.data();
        result.size = slot->total_size;

        // Mark slot as inactive to prevent message_id collision on wrap-around
        // Buffer contents remain valid until caller copies or slot is reused
        slot->active = false;
    }

    return result;
}

ReassemblySlot* ReassemblyManager::find_or_create_slot(uint16_t message_id) {
    Clock::time_point now = Clock::now();
    ReassemblySlot* oldest_slot = nullptr;
    Clock::time_point oldest_time = Clock::time_point::max();
    ReassemblySlot* empty_slot = nullptr;

    for (auto& slot : slots_) {
        // Found existing slot for this message
        if (slot.active && slot.message_id == message_id) {
            return &slot;
        }

        // Track empty slot
        if (!slot.active && !empty_slot) {
            empty_slot = &slot;
        }

        // Track oldest for LRU eviction
        if (slot.active && slot.last_activity < oldest_time) {
            oldest_time = slot.last_activity;
            oldest_slot = &slot;
        }
    }

    // Return empty slot if available
    if (empty_slot) {
        empty_slot->last_activity = now;
        return empty_slot;
    }

    // Evict oldest (LRU)
    if (oldest_slot) {
        oldest_slot->Reset();
        oldest_slot->last_activity = now;
        return oldest_slot;
    }

    return nullptr;
}

void ReassemblyManager::CleanupStale() {
    auto now = Clock::now();
    auto timeout = std::chrono::milliseconds(kReassemblyTimeoutMs);

    for (auto& slot : slots_) {
        if (slot.active && (now - slot.last_activity) > timeout) {
            slot.Reset();
        }
    }
}

int ReassemblyManager::ActiveSlotCount() const {
    int count = 0;
    for (const auto& slot : slots_) {
        if (slot.active) count++;
    }
    return count;
}

void ReassemblyManager::Reset() {
    for (auto& slot : slots_) {
        slot.Reset();
    }
}

}  // namespace imujoco::driver
